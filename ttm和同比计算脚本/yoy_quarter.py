...
# Get account_date
db.query("SELECT account_date FROM `company_finance_indicators_quarter` WHERE stock_code="+stock_code+" ORDER BY account_date DESC;");
timeSet = db.store_result();
# Traverse account_date
while True:
    ...
    if time == ():
        ...
        # Get keys
        db.query("SELECT * FROM `company_finance_indicators_quarter` WHERE stock_code="+stock_code+" ORDER BY account_date DESC;");
        ...
        for key in keys:
            # Compute yoy_quarter of key
            db.query(
       "SELECT (SELECT " + key + " FROM company_finance_indicators_quarter WHERE stock_code = "+stock_code+" AND account_date = '"+string_last_date+"') AS a, (SELECT " + key + " FROM company_finance_indicators_quarter WHERE stock_code = "+stock_code+" AND account_date = '"+string_current_time+"') AS b, (SELECT (b-a)/ABS(a));");
            indicatorsSet = db.store_result();
            indicatorsValue = indicatorsSet.fetch_row(1, 1)[0]['(SELECT (b-a)/ABS(a))'];
            # Get yoy_quarter of key
            db.query(
       "SELECT "+key+" FROM `company_finance_indicators_yoy_quarter` WHERE stock_code = "+stock_code+" AND account_date = '"+string_current_time+"'");
            ...
            # print account_date, exceptedValue, realValue
